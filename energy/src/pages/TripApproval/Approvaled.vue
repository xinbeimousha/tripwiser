        ...
        <ul class="myul">
            <van-pull-refresh class="PullRefresh" v-model="isLoading" @refresh="onRefresh">
                <van-list v-model="loading" :finished="finished" @load="onLoad">
                    <li class="myli" v-for="(item,index) in item" :key="index" v-if="item.state != -1" >
                        <ApprovalItem v-bind:item="item" @getTripDetail="getTripDetail(item)" @checkOrder ="checkOrder(item)"/>
                    </li>
                    <p class="no-result" v-if="hasNoOrder">您没有已审批的流程</p>
...
export default {
  ...
  methods: {
    checkOrder(item){
        this.$router.push({path:'/TripOrderList',query:{itemId:item.id}});
    },
    getTripDetail(item){
      this.$router.push({
          path: "/tripDetail",
    ...
    async getApprovalData(showLoading) {
      ...
      if (obj && obj.length > 0) {
        for (let i = 0; i < obj.length; i++) {
          ...
          //   obj[i].state = 4; //已过期
          //   if ((new Date().getTime() - new Date(obj[i].itinerEnd).getTime()) /1000 /60 /60 / 24 >15) {
